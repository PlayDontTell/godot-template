...
func _input(event: InputEvent) -> void:
	...
	if I.just_pressed("toggle_Dev_layer", event):
		display_debug_layer()
...
func set_time_since_start_label() -> void:
	if G.save_data:
		var time_since_start : float = G.round_to_dec(G.save_data.time_since_start, 1)
...
func set_time_played_label() -> void:
	if G.save_data:
		var time_played : float = G.round_to_dec(G.save_data.time_played, 1)
